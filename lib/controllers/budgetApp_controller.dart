...
class BudgetAppController extends GetxController {
  ...
  void deleteValueFromTransactionList() {
    if (transactionBalanceList.length > 0) {
      transactionBalanceList.removeLast();
    }
    updatePriceValue();
    update();
  }
  ...
  void addInteractionToList(String sourceName, String money, bool isLoan) {
    print(money);
    allTransactions
        .add(InteractionComp(text: sourceName, money: money, isLoan: isLoan));
    changeAddTransactionPageState();
    changeMenuState();
    update();
  }
}
